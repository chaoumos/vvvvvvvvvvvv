  ...
  try {
    ...
    querySnapshot.forEach(docSnapshot => {
      ...
      posts.push({
        ...
        description: data.description || `A post about ${data.title}`, // Ensure description for formatting
      } as BlogPost);
    });
    return posts;
  } catch (error) {
...
export async function simulateBlogCreationProcess(blogId: string, siteNameFromArgs: string): Promise<void> {
  ...
  try {
    ...
    await updateBlogStatus(blogId, 'preparing_site_structure', { githubRepoUrl, deploymentNote: `GitHub repository ${githubRepoUrl} is ready. Preparing initial site structure...` });
    console.log(`Blog ${blogId}: Status updated to preparing_site_structure.`);

    const filesToCommit: Array<{ path: string; content: string }> = [];

    const readmeContent = `# ${blogData.blogTitle}\n\nThis Hugo site for "${blogData.blogTitle}" was generated by HugoHost.\nSite Name: ${blogData.siteName}\nTheme: ${blogData.theme.name} (${blogData.theme.gitUrl})\n\nTo run locally:\n1. Clone this repository.\n2. Ensure you have Hugo installed.\n3. If using Hugo Modules for the theme, run \`hugo mod get -u\` in the repo root (after initializing modules with \`hugo mod init your.module/path\`).\n4. Run \`hugo server\`.\n`;
...
The base URL should be "https://example.com".
...
`;
    ...
    postsToPushInitially.forEach(post => {
        ...
        filesToCommit.push({ path: `content/posts/${slug}.md`, content: (global as any).formatPostToMarkdown ? (global as any).formatPostToMarkdown(post) : post.content }); // Using global hack for shared function
    });
    ...
    // Fetch posts from Firestore AFTER initial structure is pushed (or decide if dummy posts are enough)
    ...
    const deploymentNoteAfterPush = `Initial site structure (config, README, dummy posts) pushed to GitHub. Next steps: configure Cloudflare Pages or other hosting.`;
    await updateBlogStatus(blogId, 'deploying', { githubRepoUrl, deploymentNote: deploymentNoteAfterPush }); // Simulate deploying
    console.log(`Blog ${blogId}: Status updated to deploying (simulation step).`);

    await delay(2000); 

    const simulatedLiveUrl = `https://${repoNameForGithub.toLowerCase().replace(/[^a-z0-9-]/g, '')}.example-pages.dev`; 
    const finalDeploymentNote = `Hugo site structure pushed to ${githubRepoUrl}. Ready for deployment. Placeholder live URL: ${simulatedLiveUrl}.`;
  ...
  return onSnapshot(q, (querySnapshot) => {
    const posts = querySnapshot.docs.map(docSnapshot => {
      ...
      return {
        ...
        description: data.description || `A post about ${data.title}`, // Ensure description
      } as BlogPost;
    });
    onUpdate(posts);
  }, (err: any) => {
...
// This is a temporary workaround because direct function import from github.ts into firestore.ts
// can cause issues in some bundler/serverless environments if github.ts has client-side specific code
// or complex dependencies not tree-shaken correctly for a pure backend context.
// A better solution would be to ensure github.ts is fully backend-compatible or to use a shared pure-TS utility.
