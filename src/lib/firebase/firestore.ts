...
export async function simulateBlogCreationProcess(blogId: string, siteName: string): Promise<void> {
  ...
  let blogData: Blog | undefined; // Define blogData outside the try block
  ...
  try {
    ...
    blogData = blogDocSnapshot.data() as Blog; // Assign to the outer scope blogData
    ...
    const githubApiKey = apiConnections?.githubApiKey; // Use githubApiKey (PAT)
    ...
    const response = await fetch('https://api.github.com/user/repos', {
      method: 'POST',
      headers: {
        'Authorization': `token ${githubApiKey}`, // Use the fetched key
        'Content-Type': 'application/json',
        'Accept': 'application/vnd.github.v3+json',
      },
      body: JSON.stringify({
        name: siteName, 
        description: blogData.description || 'A Hugo blog generated by HugoHost', // blogData is now accessible
        private: false, 
      }),
    });
...
export async function saveApiConnection(userId: string, data: Partial<Omit<ApiConnection, 'userId'>>): Promise<void> {
  try {
    ...
    // Ensure that empty strings are saved as such, or explicitly handle to delete fields if needed.
    // For now, saving empty strings if provided by the form.
    const dataToSave = { ...data, userId: userId };

    Object.keys(dataToSave).forEach(key => {
      const typedKey = key as keyof typeof dataToSave;
      if (dataToSave[typedKey] === undefined) {
        delete dataToSave[typedKey]; // Firestore merge will ignore undefined, explicitly delete if you want to remove fields
      }
    });
...
export async function getApiConnection(userId: string): Promise<ApiConnection | null> {
  try {
    ...
    if (docSnapshot.exists()) {
      // Ensure all fields are present, even if as empty strings or undefined,
      // consistent with ApiConnection type.
      const data = docSnapshot.data();
      return {
        id: docSnapshot.id,
        userId: data.userId,
